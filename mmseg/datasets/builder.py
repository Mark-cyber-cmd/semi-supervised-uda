...
def build_dataset(cfg, default_args=None):
    ...
    from mmseg.datasets import SemiDataset
    if cfg['type'] == 'UDADataset':
        ...
        dataset = SemiDataset(
            source=build_dataset(cfg['source'], default_args),
            target_label=build_dataset(cfg['target_label'], default_args),
            target_unlabel=build_dataset(cfg['target_unlabel'], default_args),
            cfg=cfg)
...
def createDSECDataset(dataset_name, dsec_dir, gt_dir, batch_size, nr_events_data, delta_t_per_data, nr_events_window,
                        augmentation, event_representation, nr_bins_per_data, require_paired_data_train,
                        require_paired_data_val,
                        separate_pol, normalize_event, semseg_num_classes, fixed_duration,num_cpu_workers):
    """
