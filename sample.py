...
root_mask = './data/DSEC_Semantic/gt_fine/train'
file_names = os.listdir(root_sample)

a = int(len(file_names) / 16)
...
for i in range (len(file_names)):
    if file_names[i] not in sample:
        f = open('./data/DSEC_Semantic/desc_unlabel_16.txt', 'a')
...
for j in range(len(sample)):
    f = open('./data/DSEC_Semantic/desc_label_16.txt', 'a')
